import { Part } from "@/lib/data-loader";
...
export function PartDetailView({ part, locale }: Props) {
    return (
            ...
            <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    <nav className="flex items-center gap-2 text-xs font-mono text-slate-500 mb-6 print:hidden">
                        ...
                        <Link href={`/brands/${slugify(part.brand)}`} className="uppercase hover:underline">{part.brand}</Link>
                        <ChevronRight size={12} />
                        <span className="uppercase">{part.category}</span>
                        <ChevronRight size={12} />
                        <span className="text-slate-900 font-bold">{part.partNumber}</span>
                    </nav>
                    ...
                    <div className="border-b-2 border-slate-900 pb-8">
                        <div className="flex items-start justify-between mb-4">
                            ...
                            <div className="hidden md:block text-right">
                                <Link href={`/brands/${slugify(part.brand)}`} className="text-[#005EB8] font-bold text-2xl font-mono tracking-tight hover:underline">
                        ...
                        <div className="prose prose-slate max-w-none">
                            <p className="text-slate-600 leading-relaxed text-sm">
                                {part.description} This component contains high-grade alloy construction suitable for heavy-duty cycle applications. Tolerance tested to OEM specifications.
                            </p>
                        </div>
                                ...
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-200">
                                        <tr>
                                            ...
                                            <td className="px-4 py-2 font-medium text-slate-900 uppercase">{part.brand}</td>
                                        </tr>
                                        <tr>
                                            <td className="px-4 py-2 border-r border-slate-200 font-mono text-slate-600 bg-slate-50/30">CATEGORY</td>
                                            <td className="px-4 py-2 font-medium text-slate-900">{part.category}</td>
                                        </tr>
                                        {part.technical_specs && Object.entries(part.technical_specs).map(([key, val]) => (
                                            <tr key={key}>
                                            ...
                                            <tr>
                                                ...
                                                <td className="px-4 py-2 font-medium text-slate-900">
                                                    <div className="flex flex-col gap-1">
                                                        {part.oem_cross_references.map((xref, idx) => (
                    ...
                    <div className="sticky top-24">
                        <div className="bg-white border-2 border-slate-200 shadow-md p-6">
                            ...
                            <div className="mt-6 bg-slate-50 p-4 border border-slate-100 text-xs text-slate-500 mode-procurement-only">
                                ...
                                <p>Next Day Air to <span className="underline decoration-dotted">Dubai, UAE</span> available.</p>
                            </div>
