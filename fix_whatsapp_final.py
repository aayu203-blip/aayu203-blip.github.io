#!/usr/bin/env python3
"""
Script to fix WhatsApp icon by completely replacing the SVG
"""
...
def fix_whatsapp_icon(html_content):
    """Fix WhatsApp icon by completely replacing the SVG"""
    # The correct WhatsApp SVG
    correct_whatsapp_svg = '''        <svg class="relative w-8 h-8 text-white group-hover:scale-110 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24">
    ...
    # Find and replace the entire WhatsApp SVG section
    pattern = r'(<svg class="relative w-8 h-8 text-white group-hover:scale-110 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24">\s*<path d="[^"]*"[^>]*>.*?</path>\s*</svg>)'
    
    html_content = re.sub(pattern, correct_whatsapp_svg, html_content, flags=re.DOTALL)
