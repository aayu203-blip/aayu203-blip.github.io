...
# New Related Parts Template with Clear Isolation & V5 Marker
RELATED_PARTS_HTML = """
    <!-- Related Parts Section Isolated V5 -->
...
def polish_file_v5(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Clear ALL versions of Related Parts Section
    ...
    content = re.sub(r'<!-- Related Parts -->.*?<\/div>\s*<\/div>\s*<\/div>', '', content, flags=re.DOTALL)
    
    # 2. Aggressive Cleanup of Undocumented Fragments (The "Mess")
    # This targets the floating categories that lack comments
    mess_pattern = re.compile(r'<h3 class="font-bold text-gray-900 group-hover:text-yellow-600 transition-colors">Engine Components<\/h3>.*?<\/section>\s*<\/div>', re.DOTALL)
    content = mess_pattern.sub('', content)
    
    # Target another common messy fragment starting with whitespace then h3
    mess_pattern_2 = re.compile(r'\s+<h3 class="font-bold text-gray-900 group-hover:text-yellow-600 transition-colors">Engine Components<\/h3>.*?<\/section>', re.DOTALL)
    content = mess_pattern_2.sub('', content)

    # 3. Standardize WhatsApp Buttons with Inline Styles & !important
    wa_btn_pattern = re.compile(r'<a[^>]*href="https:\/\/wa\.me[^>]*>(.*?)<\/a>', re.DOTALL)
    def fix_wa_btn_v5(match):
        ...
        # If it's the fixed bottom bar WA link, keep it simple but green
        if 'fixed bottom-0' in full_tag or 'justify-center gap-2' in full_tag:
            return f"""<a href="{url}" 
               style="background-color: #25D366 !important; color: white !important; display: flex !important; align-items: center !important; justify-content: center !important; gap: 8px !important;"
        ...
        return f"""<a href="{url}" 
           style="background-color: #25D366 !important; color: white !important; display: flex !important; align-items: center !important; justify-content: center !important; gap: 12px !important;"
           class="w-full hover:bg-[#1da851] font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-3 transition-colors shadow-lg shadow-green-200/50 mb-4 group">
    ...
    content = wa_btn_pattern.sub(fix_wa_btn_v5, content)

    # 4. Cache Busting (v1.4)
    content = re.sub(r'ptc-components\.js(\?v=[\d\.]+)?', 'ptc-components.js?v=1.4', content)

    # 5. Re-inject Isolated Related Parts Section if missing
    if '</main>' in content:
        if '<!-- Related Parts Section Isolated V5 -->' not in content:
...
def main():
    ...
    broken_file = os.path.join(root_dir, 'aftermarket-scania-302082.html')
    if os.path.exists(broken_file):
        print(f"Polishing broken file (V5): {broken_file}")
        polish_file_v5(broken_file)

    files = [f for f in os.listdir(root_dir) if f.endswith('.html')]
    print(f"Starting polish V5 of {len(files)} files...")
    count = 0
    for filename in files:
        file_path = os.path.join(root_dir, filename)
        try:
            polish_file_v5(file_path)
            count += 1
            if count % 2000 == 0:
    ...
    print(f"Finished polishing {count} files with V5 (Aggressive Cleanup) polish.")
