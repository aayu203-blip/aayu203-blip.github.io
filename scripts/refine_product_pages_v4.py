...
# New Related Parts Template with Clear Isolation
RELATED_PARTS_HTML = """
    <!-- Related Parts Section Isolated -->
...
def polish_file_v4(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Clear ALL versions of Related Parts Section to ensure no mess
    ...
    # 2. Fix WhatsApp Button with direct styles (robust check)
    # Search for any wa.me link that doesn't have the green background
    wa_btn_pattern = re.compile(r'<a[^>]*href="https:\/\/wa\.me[^>]*>(.*?)<\/a>', re.DOTALL)
    def fix_wa_btn(match):
        full_tag = match.group(0)
        inner_content = match.group(1)
        if 'bg-[#25D366]' in full_tag or 'background-color: #25D366' in full_tag:
            return full_tag # Already fixed
        
        # Build the premium green button
    ...
    content = wa_btn_pattern.sub(fix_wa_btn, content)

    # 3. Clean up Header/Top bar leftovers (already handled by injector usually, but clean HTML is better)
    content = re.sub(r'<div class="w-full bg-gray-900 text-white py-3.*?<\/div>\s*<\/div>', '', content, flags=re.DOTALL)

    # 4. Cache Busting Injection (v1.3)
    content = re.sub(r'ptc-components\.js(\?v=[\d\.]+)?', 'ptc-components.js?v=1.3', content)

    # 5. Injection of Isolated Related Parts Section inside <main>
    if '</main>' in content:
        if '<!-- Related Parts Section Isolated -->' not in content:
...
def main():
    ...
    # For now, let's target the reported broken file first, then others
    broken_file = os.path.join(root_dir, 'aftermarket-scania-302082.html')
    if os.path.exists(broken_file):
        print(f"Polishing broken file: {broken_file}")
        polish_file_v4(broken_file)

    files = [f for f in os.listdir(root_dir) if f.endswith('.html')]
    print(f"Starting polish V4 of {len(files)} files...")
    count = 0
    for filename in files:
        file_path = os.path.join(root_dir, filename)
        try:
            polish_file_v4(file_path)
            count += 1
            if count % 1000 == 0:
    ...
    print(f"Finished polishing {count} files with V4 (Layout Containment) polish.")
