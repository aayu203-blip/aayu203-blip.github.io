...
# New Related Parts Template with Clear Isolation & V7 Marker
# Note: We wrap it in 3 closing divs to catch any unclosed grid/column traps
RELATED_PARTS_HTML = """
    <!-- Force-Close Structural Traps -->
    </div></div></div>
    <!-- Related Parts Section Isolated V7 -->
    <div style="clear: both; width: 100%; display: block; background: white;">
        <section class="mt-16 pt-12 border-t border-gray-200">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 px-4">
...
def polish_file_v7(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. NUKE ALL STRUCTURAL NOISE
    ...
    # 4. Standardize WhatsApp Buttons with Extreme Visibility
    wa_btn_pattern = re.compile(r'<a[^>]*href="https:\/\/wa\.me[^>]*>(.*?)<\/a>', re.DOTALL)
    def fix_wa_btn_v7(match):
    ...
    content = wa_btn_pattern.sub(fix_wa_btn_v7, content)

    # 5. Cache Busting (v1.6)
    content = re.sub(r'ptc-components\.js(\?v=[\d\.]+)?', 'ptc-components.js?v=1.6', content)

    # 6. Re-inject Isolated Related Parts Section with Force-Close Tags
    if '</main>' in content:
        if '<!-- Related Parts Section Isolated V7 -->' not in content:
...
def main():
    ...
    # Test on specifically reported broken pages
    priority = ['aftermarket-scania-323790.html', 'aftermarket-scania-302082.html']
    for p in priority:
        file_path = os.path.join(root_dir, p)
        if os.path.exists(file_path):
            print(f"Polishing high-priority file (V7 Nuclear): {p}")
            polish_file_v7(file_path)

    files = [f for f in os.listdir(root_dir) if f.endswith('.html')]
    print(f"Starting Nuclear polish V7 of {len(files)} files...")
    count = 0
    ...
    print(f"Finished polishing {count} files with V7 (Nuclear) polish.")
